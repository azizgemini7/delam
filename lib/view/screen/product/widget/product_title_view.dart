        ...
        builder: (context, details, child) {
          return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Hero(
              ...
              child: Text(
                '${_startingPrice}',
                // '${PriceConverter.convertPrice(context, _startingPrice)}''${_endingPrice!= null ? ' - ${PriceConverter.convertPrice(context, _endingPrice)}' : ''}',
...
class ProductTitleViewFireBase extends StatelessWidget {
  final Product productModel;
  ProductTitleViewFireBase({@required this.productModel});
  ...
  Widget build(BuildContext context) {
    ...
    double _startingPrice;
    double _endingPrice;
    if(productModel.choiceOptions != null && productModel.choiceOptions.length != 0) {
      List<double> _priceList = [];
      productModel.variation.forEach((variation) => _priceList.add(double.parse(variation.price)));
      _priceList.sort((a, b) => a.compareTo(b));
      _startingPrice = _priceList[0];

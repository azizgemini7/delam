...
class ProductProvider extends ChangeNotifier {
  ...
  initBrandOrCategoryProductList(bool isBrand, String id) async {
    _brandOrCategoryProductList.clear();
    _hasData = true;
    productRepo.getBrandOrCategoryProductList().forEach((product) {
      product.categoryIds.forEach((element) {
        if(element.id == id){
          _brandOrCategoryProductList.add(product);
        }
      });
    });
  ...
  void initRelatedProductList() async {
    ...
    ///MALAS ERROR (setState() or markNeedsBuild() called during build.)
    // notifyListeners();
  }

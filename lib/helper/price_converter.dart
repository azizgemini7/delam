...
class PriceConverter {
  static String convertPrice(BuildContext context, double price, {String discount, String discountType}) {
   ...
   // return '${Provider.of<SplashProvider>(context, listen: false).currency.symbol}${(price * double.parse(Provider.of<SplashProvider>(context, listen: false).currency.exchangeRate)).toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
